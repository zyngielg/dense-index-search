...
class MedQACorpus():
    ...
    def filter_textbooks(self):
        ...
        for book_title_txt in os.listdir(self.textbooks_dir):
            ...
            with open(textbook_path, 'r') as textbook_file:
                ...
                num_words_before = sum([len(word_tokenize(x)) for x in sentences])
                # remove
                ...
                num_words_after = sum([len(word_tokenize(x)) for x in sentences])
                ...
                self.__log_message(book_title_txt[:-4], num_sentences_before, num_sentences_after, num_words_before, num_words_after)
    ...
    def __log_message(self, title, num_sent_before, num_sent_after, num_words_before, num_words_after):
        log_message = f"[{title}] \n\t#sentences: \tbefore: {num_sent_before}\tafter: {num_sent_after} -> removed: {num_sent_before-num_sent_after}\n"
        log_message += f"\tRemoved {self.short_sentence_counter} sentences which had less than 6 words and after {self.long_word_sentences_counter} which contained a word of length over 35"

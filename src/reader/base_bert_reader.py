...
from models.BaseBERTLinear import BertForMultipleChoice
...
class Base_BERT_Reader(Reader):
    ...
    weights_file_name = "final_BERT_reader.pth"
    ...
    def __init__(self, load_weights=False):
        ...
        self.model = BertForMultipleChoice(self.bert_name)
        if load_weights:
            saved_model = torch.load(self.weights_file_path)
            saved_model = {key.replace(
                "module.", ""): value for key, value in saved_model.items()}
